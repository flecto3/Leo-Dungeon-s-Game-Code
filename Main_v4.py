...
ASSETS_FOLDER = os.path.join(SCRIPT_DIR, "assets")  # Folder containing music files
...
class Game:
    ...
    def play_music(self, music_name: str):
        ...
        try:
            ...
            # Debug: Show what path we're looking for (only first time)
            if not hasattr(self, '_shown_music_path'):
                print(f"[MUSIC] Looking for music files in: {ASSETS_FOLDER}")
                self._shown_music_path = True
    ...
    def play_intro_music(self):
        """Play the intro/mode select screen music"""
        self.play_music("intro_music.mp3")
    ...
    def run(self):
        # Play intro music when game starts
